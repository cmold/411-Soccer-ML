...
def main():
    ...
    for gd_mult in gd_vals:    
        ...
        gd_accuracy_list.append(accuracy)
    
    gd_optimize = np.array(gd_optimize_list)
    gd_accuracy = np.array(gd_accuracy_list)
    
    plot_optimal(gd_optimize, gd_accuracy)
    
    gd_optimal = optimize(gd_optimize, gd_accuracy)
    
    # Get optimized results
    res1, accuracy, gd_mult = ML_prediction(data, teams, KVAL, gd_optimal)
    
    print(res1)
    print('Model accuracy: %s percent' %(accuracy*100))
