...
def preproccessing(dataframe):
    ...
    dummy_home = pd.get_dummies(
        soccer_no_dup.HomeTeam, drop_first=True, prefix='HomeTeam')
    dummy_away = pd.get_dummies(
        soccer_no_dup.AwayTeam, drop_first=True, prefix='AwayTeam')
    dummy_results = pd.get_dummies(
        soccer_no_dup.FTR, drop_first=True, prefix='FTR')
    dummy_halftime_res = pd.get_dummies(
        soccer_no_dup.HTR, drop_first=True, prefix='HTR')
    dummy_referee = pd.get_dummies(
        soccer_no_dup.Referee, drop_first=True, prefix='Referee')
    ...
    soccer_no_dup['Halftime Goal Difference'] = soccer_no_dup.HTHG.sub(
        soccer_no_dup.HTAG)
    soccer_no_dup['Shot Difference'] = soccer_no_dup.HS.sub(soccer_no_dup.AS)
    soccer_no_dup['Shot On Target Difference'] = soccer_no_dup.HST.sub(
        soccer_no_dup.AST)
    ...
    soccer_no_dup['Yellow Card Difference'] = soccer_no_dup.HY.sub(
        soccer_no_dup.AY)
...
def run_model(data):
    ...
    X = data.drop(columns=['Div', 'Date', 'FTHG', 'FTAG', 'FTR_D', 'FTR_H'])
    y = data[['FTR_D', 'FTR_H']]
